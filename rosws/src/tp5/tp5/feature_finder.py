...
class LandmarkObserver(Node):
    def __init__(self):
        ...
        self.sub_map = self.create_subscription(PoseArray, '/landmarks', self.map_callback, qos)
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        for landmark in self.map_landmarks.poses:
            ...
            if angle_min <= theta_landmark <= angle_max and r_min <= r_landmark <= r_max:
                ...
                if visible:
                    pose.position.x = float(r_landmark + np.random.normal(0, 0.005))  # add small noise
                    pose.position.y = float(0.0)
                    pose.position.z = float(theta_landmark + np.random.normal(0, 0.005))  # add small noise
                else:
